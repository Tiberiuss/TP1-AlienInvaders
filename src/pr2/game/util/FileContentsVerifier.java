package pr2.game.util;

import pr2.game.Game;
import pr2.game.Level;
import pr2.game.Move;
...
public class FileContentsVerifier {
	...
	public boolean verifyCycleString(String cycleString) {
		...
		if (words.length != 3 || !verifyCurrentCycle(Integer.parseInt(words[1])))
			return false;
		return true;
	}
	...
	public boolean verifyPlayerString(String lineFromFile, Game game, int armour) {
		...
		if (!verifyCoords(Integer.parseInt(coords[0]), Integer.parseInt(coords[1]), game)
				|| !verifyLives(Integer.parseInt(words[2]), armour) || !verifyPoints(Integer.parseInt(words[3]))
				|| !verifyBool(words[5])
				|| !verifyBool(words[6]))
			return false;
		return true;
	...
	public boolean verifySmartBombString(String lineFromFile, Game game) {
		String[] words = lineFromFile.split(separator1);
		if (words.length != 3)
			return false;
